...
| run_once_40_tmux-tpm.sh.tmpl        | run once               | Instalar TPM (Tmux Plugin Manager) em ~/.tmux/plugins/tpm                                        |
...
- `echo $HISTFILE` — deve ser `~/.zsh_history`.
...
- **Histórico não persiste:** Falha de escrita em `$HOME` (permissões, disco cheio). Correção: corrigir permissões/armazenamento.
