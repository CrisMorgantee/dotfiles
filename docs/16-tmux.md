# Tmux: auto-session e status com Git

## Objetivo

Documentar a integração do tmux com este ambiente: sessão automática por hostname em SSH, status-right com branch e ahead/behind do Git, TPM e plugins (nordtheme, tmux-yank).
...
- **Git no status-right:** O script `tmux-git-status` recebe o diretório do pane atual e imprime branch, indicador de dirty (*) e ahead/behind (⇡/⇣). Leve (bash + git), sem plugin pesado; atualização conforme `status-interval` (5s no dot_tmux.conf).
...
|---------------------------------------|---------------------------|-----------|
| dot_tmux.conf                         | ~/.tmux.conf              | Config: status-left/right, interval 5, keybindings, TPM, nordtheme, tmux-yank. status-right chama tmux-git-status com pane_current_path. |
...
| dot_local/bin/executable_tmux-git-status | ~/.local/bin/tmux-git-status | Recebe diretório; sai em 0 se não for repo git; senão imprime branch (ou short SHA), * se dirty, ⇡N ⇣M se upstream. |
...
- **Dentro do tmux:** status-right a cada 5s chama tmux-git-status com o path do pane atual; exibe algo como `main* ⇡1 ⇣0` ou só a data/hora se não for repo git.
...
- Dentro de tmux em repo git: status-right mostra branch e, se houver upstream, ⇡/⇣.
- `which tmux-auto tmux-git-status` resolve para ~/.local/bin (PATH do .zshrc).
...
- **Status-right vazio ou sem git:** tmux-git-status só imprime em repo git; fora dele sai 0 sem output. status-interval alto pode atrasar atualização.
...
- Remover git do status: em dot_tmux.conf, reverter status-right para apenas data/hora (ex.: `set -g status-right "%Y-%m-%d %H:%M "`).
